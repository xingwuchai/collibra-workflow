...
USER_LIMIT = 1000
TECHNICAL_DATA_STEWARD_ROLE_ID = '88a09674-e580-4323-9d64-953f493ac156'
...
def findTechnicalStewardsForColumn(Asset column) {
  List responsibilities = findResponsibilitiesForRoleOnResourceWithHierarchy(string2Uuid(TECHNICAL_DATA_STEWARD_ROLE_ID), column)
...
String prefix = 'scripttask, identifyTechnicalDataStewards, '
...
reportIds.each { reportId ->
  ...
  List responsibilities = findResponsibilitiesForRoleOnResource(string2Uuid(TECHNICAL_DATA_STEWARD_ROLE_ID), report)
  List validators = getUsersFromResponsibilities(responsibilities, USER_LIMIT)
  if (validators.isEmpty()) {
  ...
  if (!validators.isEmpty()) {
    ...
    loggerApi.warn('couldn\'t find a suitable technical data steward for the report')
  }
}
